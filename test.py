...
#215363 requests; 29.63%
total = 0
# finds number of requests with 4XX error codes
number_of_line = 0
with open('httpaccesslog.txt') as f:
        ...
        print(total)
        print("percentage: ", (total / number_of_line) * 100)
                      
#     4. What percentage of the requests were redirected elsewhere (any 3xx codes)?
# 307262 requests; 42.28%
total = 0
# finds number of requests with 3XX error codes
number_of_line = 0
with open('httpaccesslog.txt') as f:
        for line in f:
                for x in range(100):
                        finded = line.find(str(x + 300))
                        if finded != -1:
                                total += 1
                                break
                number_of_line += 1
...
with open('httpaccesslog.txt') as f:
        files = []
        for line in f:
                files.append(line.findall(r"(.*?\..*?)"))
        most = Counter(files.upper() for file in files)
        print(most)
